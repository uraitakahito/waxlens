...
interface WaxlensReport {
  ...
  /** Identity of the validated WACZ. See `ReportSource` below. */
  source: ReportSource;
...
type ReportSource =
  | { kind: "file"; path: string }   // absolute, canonical path on local fs
  | { kind: "s3"; uri: string };     // s3://bucket/key URI
```
...
`source` は producer-agnostic な validation identity を表す
discriminated union。`kind: "file"` の `path` は常に絶対パス
(`path.resolve()` で canonicalize された値)、`kind: "s3"` の `uri` は
`parseS3Uri` が validate した `s3://bucket/key` 形式。caller は kind で
分岐すれば安全に display / dedup / DB 永続化に使える。
...
```json
{
  "waxlensVersion": "0.0.0",
  "profile": "spec",
  "source": { "kind": "s3", "uri": "s3://my-bucket/captures/abc123.wacz" },
  "valid": true,
  "summary": { "passed": 12, "failed": 0, "warnings": 0, "info": 0, "durationMs": 89 },
  "issues": [],
...
```json
{
  "waxlensVersion": "0.0.0",
  "profile": "spec",
  "source": { "kind": "file", "path": "/tmp/bad-hash.wacz" },
  "valid": false,
  "summary": { "passed": 11, "failed": 1, "warnings": 0, "info": 0, "durationMs": 14 },
  "issues": [
    {
