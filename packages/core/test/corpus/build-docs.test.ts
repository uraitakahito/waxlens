 ...
 *
 *   # 生成 (docs を書き換える)
 *   CORPUS_DIR=<corpus の絶対パス> pnpm --filter @wacz-validator/core corpus:docs
 *   # 鮮度検査 (差分があれば fail。CI 用)
 *   CORPUS_DIR=<corpus の絶対パス> pnpm --filter @wacz-validator/core corpus:docs:check
 */
...
// WACZ_VALIDATOR_DOCS_CHECK=1 のときは書き換えず、committed な docs と突き合わせる。
const checkMode = process.env["WACZ_VALIDATOR_DOCS_CHECK"] === "1";
...
// corpus 側へ書く。依存の向きは wacz-validator → corpus のまま変わらない。
  ...
  it.each(targets(corpusRootDir ?? ""))(
    ...
    async ({ lang, path, label }) => {
      ...
      if (checkMode) {
        expect(
          current,
          `${label} が manifest と不一致。corpus repo で \`pnpm --filter @wacz-validator/core corpus:docs\` を実行して commit してください`,
        ).toBe(next);
